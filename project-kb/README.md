...
- **`skills/`** — the canonical skill files. These tell an LLM how to perform KB tasks (process inbox, generate outputs, compile release notes). When `atami kb init` runs in a project, these get copied into that project's `.project-kb/skills/` directory.
...
The skill files in `skills/` are the source of truth. Edit them here, commit, and the changes will propagate to projects when they run `atami skills pull`.
